...
class DashboardInfureController extends Controller
{
        ...
        $query = "
            ...
            LEFT JOIN (
                ...
                LEFT JOIN tdjamkerja_jammatimesin tjkjmm ON tjkjmm.jam_kerja_mesin_id = tdjamkerjamesin.id
                    AND tjkjmm.jam_mati_mesin_id != 10 -- filter jam mati mesin yang bukan tidak ada schedule
    ...
    */
    ...
    public function getTotalProductionMonthly(Request $request)
    {
        ...
        // Aggregate tdjamkerjamesin per machine per period (restrict to factory machines) and join to machines
        $totalProductionMonthly = collect(DB::select('
                ...
                LEFT JOIN (
                    SELECT
                        ...
                        END AS period_ke,
                        COALESCE(SUM(tpa.berat_produksi),0) AS berat_produksi
                    FROM tdproduct_assembly tpa
                    WHERE tpa.production_date BETWEEN :startMonth AND :endMonth
                    GROUP BY tpa.machine_id, period_ke
            ...
            'firstPeriodPlus' => Carbon::parse($firstPeriod)->format('Y-m-d 07:01:00'),
            'secondPeriod'    => $secondPeriod,
            'secondPeriodPlus' => Carbon::parse($secondPeriod)->format('Y-m-d 07:01:00'),
            'endMonth'        => $endMonth,
        ]));
    ...
    public function getPeringatanKatagae(Request $request)
    {
        ...
        ', [
            ...
            return [
                ...
                'sisa_meter' => $item->sisa_meter * -1, // convert to negative value to indicate remaining length
                'berat' => $berat,
                'jam' => floor($menit / 60),
                'menit' => $menit % 60,
            ];
    ...
    public function getLossMonthly(Request $request)
    {
        ...
        $produksiLossMonthly = collect(DB::select('
            SELECT
                ...
                RIGHT(mac.machineno, 2) AS machineno,
                periods.period_ke,
                COALESCE(ROUND(SUM(tpa.berat_produksi)::numeric, 1), 0) AS berat_produksi,
                COALESCE(ROUND(SUM(tpaloss.berat_loss)::numeric, 1), 0) AS berat_loss
            FROM msmachine mac
            CROSS JOIN (VALUES (1), (2), (3)) AS periods(period_ke)
            LEFT JOIN tdproduct_assembly tpa
                ON mac.id = tpa.machine_id
    ...
    public function getProductionMonthly(Request $request)
    {
        ...
        $produksiLossMonthly = collect(DB::select('
            ...
            WHERE mac.department_id = :factory
                AND mac.status = 1
            GROUP BY mac.id, mac.machineno, periods.period_ke
            ORDER BY RIGHT(mac.machineno, 2) ASC, periods.period_ke ASC
        ', [
            'factory'         => $request->factory,
            'startMonth'      => $startMonth,
            'firstPeriod'     => $firstPeriod,
