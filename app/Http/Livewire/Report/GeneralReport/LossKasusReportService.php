    ...
    public static function daftarLossPerMesinJenis($nipon, $jenisReport, $tglMasuk, $tglKeluar)
    {
        ...
        if ($nipon === 'Infure') {
            ...
            $data = DB::table('msmachine')
                ...
                ->get();
        } else if ($nipon === 'Seitai') {
            $produksiPerMesin = DB::table('tdproduct_goods as tpg')
                ->select('tpg.machine_id', DB::raw('COALESCE(SUM(tpg.qty_produksi), 0) AS total_produksi'))
                ->whereBetween('tpg.production_date', [$tglMasuk, $tglKeluar])
                ->groupBy('tpg.machine_id');
            ...
            $data = DB::table('msmachine')
                ->select(
                    ...
                    'msmachine.machinename',
                    DB::raw('COALESCE(produksi.total_produksi, 0) AS total_produksi'),
                    DB::raw('COALESCE(SUM(tpgl.berat_loss), 0) AS total_loss')
                )
                ->crossJoin('mslossseitai as msloss')
                ->where('msloss.status', 1)
                ->leftJoin('tdproduct_goods as tpg', function ($join) use ($tglMasuk, $tglKeluar) {
                    $join->on('tpg.machine_id', '=', 'msmachine.id')
                        ->whereBetween('tpg.production_date', [$tglMasuk, $tglKeluar]);
                })
                ->leftJoin('tdproduct_goods_loss as tpgl', function ($join) {
                    $join->on('tpgl.product_goods_id', '=', 'tpg.id')
                        ->on('tpgl.loss_seitai_id', '=', 'msloss.id');
                })
                ->leftJoinSub($produksiPerMesin, 'produksi', function ($join) {
                    $join->on('produksi.machine_id', '=', 'msmachine.id');
                })
                ->leftJoin('msdepartment as msdep', 'msdep.id', '=', 'msmachine.department_id')
                ->where('msmachine.status', 1)
                ->whereIn('msmachine.department_id', departmentHelper::seitaiDepartment())
                ->groupBy(
                    'msloss.id',
                    'msloss.code',
                    'msmachine.id',
        ...
        if ($nipon === 'Infure') {
            ...
            $data = DB::table('msmachine')
                ->select(
                    ...
                    'msmachine.machinename',
                    DB::raw('COALESCE(produksi.total_produksi, 0) AS total_produksi'),
                    DB::raw('COALESCE(SUM(tpal.frekuensi), 0) AS frekuensi_loss')
                )
                ->crossJoin('mslossinfure as msloss')
                ->where('msloss.status', 1)
                ->leftJoin('tdproduct_assembly as tpa', function ($join) use ($tglMasuk, $tglKeluar) {
            ...
            $data = DB::table('msmachine')
                ->select(
                    ...
                    'msmachine.machinename',
                    DB::raw('COALESCE(produksi.total_produksi, 0) AS total_produksi'),
                    DB::raw('COALESCE(SUM(tpgl.frekuensi), 0) AS frekuensi_loss')
                )
                ->crossJoin('mslossseitai as msloss')
                ->where('msloss.status', 1)
                ->leftJoin('tdproduct_goods as tpg', function ($join) use ($tglMasuk, $tglKeluar) {
