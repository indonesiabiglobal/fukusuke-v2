...
class ProduksiPerTypeReportService
{
    public static function daftarProduksiPerTipeInfure($nippon, $jenisReport, $tglMasuk, $tglKeluar)
    {
        ...
        foreach ($listProductGroup as $productGroupCode => $productGroup) {
            ...
            $columnItem++;
            // weight rate
            $activeWorksheet->setCellValue($columnItem . $rowItem, $dataItem->berat_standard > 0 ? '=' . $columnBeratProduksi . $rowItem . '/' . $columnBeratStandard . $rowItem . '*100' : 0);
        ...
        phpspreadsheet::textAlignCenter($spreadsheet, $columnProductGroupCode . $startRowItem . ':' . $columnProductGroupCode . ($rowItem - 1));
        ...
        $columnItem++;
        // loss (%)
        $spreadsheet->getActiveSheet()->setCellValue($columnItem . $rowGrandTotal, $grandTotal['berat_produksi'] > 0 ? $grandTotal['infure_berat_loss'] / $grandTotal['berat_produksi'] * 100 : 0);
        phpspreadsheet::numberFormatCommaSeparated($spreadsheet, $columnItem . $rowGrandTotal);
        $columnItem++;
        // panjang produksi
        ...
        $spreadsheet->getActiveSheet()->getColumnDimension('B')->setWidth(6); // Type code
    ...
    public static function daftarProduksiPerTipeSeitai($nippon, $jenisReport, $tglMasuk, $tglKeluar)
    {
        ...
        foreach ($listProductGroup as $productGroupCode => $productGroup) {
            ...
            $columnItem++;
            // seitai loss %
            $activeWorksheet->setCellValue($columnItem . $rowItem, $dataItem->berat_produksi > 0 ? $dataItem->seitai_berat_loss / $dataItem->berat_produksi * 100 : 0);
            phpspreadsheet::numberFormatCommaSeparated($spreadsheet, $columnItem . $rowItem);
            $columnItem++;
            // seitai_cost
        ...
        $columnItem++;
        // seitai loss %
        $spreadsheet->getActiveSheet()->setCellValue($columnItem . $rowGrandTotal, $grandTotal['berat_produksi'] > 0 ? $grandTotal['seitai_berat_loss'] / $grandTotal['berat_produksi'] * 100 : 0);
        phpspreadsheet::numberFormatCommaSeparated($spreadsheet, $columnItem . $rowGrandTotal);
        $columnItem++;
        ...
        $columnWidths = [12, 12, 8, 8, 12, 12, 12]; // Lebar untuk setiap kolom data Seitai
        $tempColumn = $startColumnItemData;
        foreach ($columnWidths as $width) {
            if (ord($tempColumn) <= ord($endColumnItem)) {
                $spreadsheet->getActiveSheet()->getColumnDimension($tempColumn)->setWidth($width);
