...
class JenisProduk extends Component
{
    ...
    {
        $response = $this->exportJenisProduk();
    ...
    public function exportJenisProduk()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER JENIS PRODUK - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $data = DB::table('msproduct_group')
            ->select('code', 'name', 'status', 'updated_by', 'updated_on')
            ->orderBy('code', 'ASC')
            ->get();
        ...
        $filename = 'Master-Jenis-Produk.xlsx';
