...
class MachinePartDetailController extends Component
{
    ...
    {
        $response = $this->exportDetailBagianMesin();
    ...
    public function exportDetailBagianMesin()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER DETAIL BAGIAN MESIN - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $header = ['No', 'Kode', 'Nama', 'Bagian Mesin', 'Departemen', 'Status', 'Updated By', 'Updated On'];
        ...
        $data = DB::table('ms_machine_part_detail as mspd')
            ->select('mspd.code', 'mspd.name', 'msp.part_machine', 'msd.name as departmentname', 'mspd.status', 'mspd.updated_by', 'mspd.updated_on')
            ->leftJoin('ms_machine_part as msp', 'msp.id', '=', 'mspd.machine_part_id')
            ->leftJoin('msdepartment as msd', 'msd.id', '=', 'msp.department_id')
            ->orderBy('mspd.code', 'ASC')
            ->get();
        ...
        $filename = 'Master-Detail-Bagian-Mesin.xlsx';
