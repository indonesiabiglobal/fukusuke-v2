...
class MasalahKenpinInfureController extends Component
{
    ...
    {
        $this->startHeavyJob();
        $response = $this->exportMasalahKenpinInfure();
    ...
    private function exportMasalahKenpinInfure()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER MASALAH KENPIN INFURE - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $departments = departmentHelper::masalahKenpinInfureDepartmentGroup();
        $data = MsMasalahKenpin::where('department_group_id', $departments[0]->id)
            ->orderBy('code', 'ASC')
            ->get();
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="Master-Masalah-Kenpin-Infure.xlsx"');
