...
class MenuLossKlasifikisasiController extends Component
{
    ...
    {
        $this->startHeavyJob();
        $response = $this->exportLossKlasifikasi();
    ...
    private function exportLossKlasifikasi()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER LOSS KLASIFIKASI - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $data = MsLossClass::orderBy('code', 'ASC')->get();
        ...
        foreach ($data as $item) {
            ...
            $activeWorksheet->setCellValue('F' . $row, $item->updated_on);
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="Master-Loss-Klasifikasi.xlsx"');
