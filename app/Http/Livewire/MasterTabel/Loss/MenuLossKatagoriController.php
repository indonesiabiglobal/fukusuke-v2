...
class MenuLossKatagoriController extends Component
{
    ...
    {
        $this->startHeavyJob();
        $response = $this->exportLossKategori();
    ...
    private function exportLossKategori()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER LOSS KATEGORI - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $data = MsLossCategory::orderBy('code', 'ASC')->get();
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="Master-Loss-Kategori.xlsx"');
