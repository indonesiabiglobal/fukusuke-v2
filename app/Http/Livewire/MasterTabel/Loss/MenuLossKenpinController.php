...
class MenuLossKenpinController extends Component
{
    ...
    {
        $this->startHeavyJob();
        $response = $this->exportLossKenpin();
    ...
    private function exportLossKenpin()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER LOSS KENPIN - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $data = MsLossKenpin::join('mslosscategory as mlc', 'mslosskenpin.loss_category_code', '=', 'mlc.code')
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="Master-Loss-Kenpin.xlsx"');
