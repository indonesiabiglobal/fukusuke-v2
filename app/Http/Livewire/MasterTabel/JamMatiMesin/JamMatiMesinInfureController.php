...
class JamMatiMesinInfureController extends Component
{
    ...
    {
        $this->startHeavyJob();
        $response = $this->exportJamMatiMesinInfure();
    ...
    private function exportJamMatiMesinInfure()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'MASTER JAM MATI MESIN INFURE - ' . Carbon::now()->translatedFormat('M Y'));
        ...
        $data = MsJamMatiMesin::where('department_id', 2)->orderBy('code', 'ASC')->get();
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="Master-Jam-Mati-Mesin-Infure.xlsx"');
        return ['status' => 'success', 'spreadsheet' => $response];
    }

    public function render()
    {
        $query = MsJamMatiMesin::query()->where('department_id', 2);
        ...
        return view('livewire.master-tabel.jam-mati-mesin.infure', [
            'result' => $result,
