                ...
                <div class="col-12">
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-4 mt-1">
                            <div class="form-group">
                                <div class="input-group">
                                    <label class="control-label col-5 pe-2">Timbangan ID</label>
                                    <select class="form-control @error('timbangan_id') is-invalid @enderror"
                                        wire:model="timbangan_id" id="timbangan-select">
                                        <option value="">- Pilih Timbangan -</option>
                                        <option value="VH-M1">VH-M1</option>
                                        <option value="VH-M2">VH-M2</option>
                                        <option value="VH-M3">VH-M3</option>
                                        <option value="VH-M4">VH-M4</option>
                                    </select>
                                    @error('timbangan_id')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    <button type="button" class="btn btn-sm"
                                        :class="$wire.timbangan_id ? 'btn-success' : 'btn-secondary'"
                                        id="connect-websocket"
                                        :disabled="!$wire.timbangan_id">
                                        <i class="ri-wifi-line"></i> <span id="ws-status">Hubungkan</span>
                                    </button>
                                </div>
                            </div>
                        </div>
...
@script
    <script>
        ...
        // Laravel Broadcasting Connection untuk Timbangan
        let echoChannel = null;
        let isConnected = false;
        const connectButton = document.getElementById('connect-websocket');
        const wsStatusText = document.getElementById('ws-status');
        const timbanganSelect = document.getElementById('timbangan-select');

        if (connectButton) {
            connectButton.addEventListener('click', function() {
                const timbanganId = timbanganSelect.value;

                if (!timbanganId) {
                    alert('Silakan pilih Timbangan ID terlebih dahulu');
                    return;
                }

                if (isConnected && echoChannel) {
                    // Disconnect
                    window.TimbanganWebSocket.disconnect();
                    echoChannel = null;
                    isConnected = false;
                    connectButton.classList.remove('btn-danger');
                    connectButton.classList.add('btn-success');
                    wsStatusText.textContent = 'Hubungkan';
                    console.log('Disconnected from channel:', timbanganId);
                } else {
                    // Connect
                    connectToChannel(timbanganId);
                }
            });
        }

        function connectToChannel(timbanganId) {
            try {
                // Subscribe ke channel berdasarkan timbangan_id menggunakan WebSocket
                echoChannel = window.TimbanganWebSocket.channel(`timbangan.${timbanganId}`)
                    .listen('weight.received', (data) => {
                        console.log('Data received from WebSocket:', data);

                        // Update berat_produksi via Livewire
                        if (data.berat !== undefined) {
                            $wire.call('updateBeratFromTimbangan', data.berat);

                            // Optional: Show notification
                            console.log(`Berat diterima: ${data.berat} kg dari ${data.timbangan_id}`);
                        }
                    });
                ...
                wsStatusText.textContent = 'Putuskan';
                console.log('Connected to WebSocket channel:', timbanganId);

            } catch (e) {
                console.error('Connection error:', e);
                alert('Error menghubungkan: ' + e.message);
            }
        }
